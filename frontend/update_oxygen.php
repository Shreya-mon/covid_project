...
<?php 
	$qry = mysqli_query($link, "SELECT * FROM `oxygen_db` WHERE `id` = '".$_REQUEST['id']."'");
		$row = mysqli_fetch_array($qry);
        $type = isset($row['application type']) ? $row['application type']:'';
...
<body>
	<center>
	<h1>Update Your Record As Oxygen Supplier</h1>
	...
	<form action="#" method="post">
        ...
        Organisation <input type="radio" name="person" value="Organisation" <?php if($type == 'Organisation'){echo "checked";} ?>>
    <br><br>
...
$qry = mysqli_query($link, "UPDATE `oxygen_db` SET `name`='".$_REQUEST['name']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `oxygen_db` SET `application type`='".$_REQUEST['person']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `oxygen_db` SET `address`='".$_REQUEST['address']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `oxygen_db` SET `district`='".$_REQUEST['district']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `oxygen_db` SET `phone number`='".$_REQUEST['phno']."' WHERE `id`='".$_REQUEST['id']."'");
$qry1 = mysqli_query($link, "UPDATE `oxygen_db` SET `proof`='".$_REQUEST['proof']."' WHERE `id`='".$_REQUEST['id']."'");
if ($qry) {
