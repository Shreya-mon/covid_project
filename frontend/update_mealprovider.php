...
<?php 
	$qry = mysqli_query($link, "SELECT * FROM `mealprovider_db` WHERE `id` = '".$_REQUEST['id']."'");
		$row = mysqli_fetch_array($qry);
        $dist = isset($row['district']) ? $row['district']:'';
?>
<?php
if(isset($_POST['submit'])){
$qry = mysqli_query($link, "UPDATE `mealprovider_db` SET `name`='".$_REQUEST['name']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `mealprovider_db` SET `address`='".$_REQUEST['address']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `mealprovider_db` SET `district`='".$_REQUEST['district']."' WHERE `id`='".$_REQUEST['id']."'");
$qry = mysqli_query($link, "UPDATE `mealprovider_db` SET `phno`='".$_REQUEST['phno']."' WHERE `id`='".$_REQUEST['id']."'");
$qry1 = mysqli_query($link, "UPDATE `mealprovider_db` SET `proof`='".$_REQUEST['proof']."' WHERE `id`='".$_REQUEST['id']."'");
if ($qry) {
...
<div class="container pt-5 pb-5">
        <div class="row pt-3 pb-3 text-center" style="background-color: #c368ff; color: #FFFFFF">
            ...
            <h2 class="col-10">Update Your Record As Meal Provider</h2>
        </div>
